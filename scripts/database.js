...
const filterWalkerCitiesByWalker = (walkerObject) => {
    ...
    const assignmentObjectsArray = [ ]
    // Iterate the array value of walkerCities
    for (walkerCity of walkerCities) {
        ...
        if (walkerCity.walkerId === walker.id) {
            ...
            assignmentObjectsArray.push(walkerCity)
        }
        
        // After the loop is done, return the assignments array
    }
    return assignmentObjectsArray
    //Remember to set this return in a varialble to use
}

//The parameter for this is the returned result from
//filterWalkerCitiesByWalker
const assignCityNames = (assignmentObjectsArray) => {
    let returnedString = " "
    
    for (const assignment of assignmentObjectsArray) {
        
        for (city of cities) {
            
            
            if (city.id === assignment.cityId) {
            let cityNames = `${city.name} and ${city.name}`
            //
            }
        }
        return cityNames
...
/*let assign = filterWalkerCitiesByWalker(walker)
let namesCity = cityName(assign)
